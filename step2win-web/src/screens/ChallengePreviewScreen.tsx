...
const THEME: Record<string, { bg: string; accent: string }> = {
  blue: { bg: 'hsl(var(--bg-card))', accent: '#4F9CF9' },
  green: { bg: 'hsl(var(--bg-card))', accent: '#34D399' },
  purple: { bg: 'hsl(var(--bg-card))', accent: '#A78BFA' },
  orange: { bg: 'hsl(var(--bg-card))', accent: '#FB923C' },
  pink: { bg: 'hsl(var(--bg-card))', accent: '#F472B6' },
};
...
export default function ChallengePreviewScreen() {
  ...
  return (
    <div className="min-h-screen pb-32 bg-bg-page">
      <div className="relative px-4 pt-6 pb-8" style={{ background: theme.bg }}>
        <button
          onClick={() => navigate(-1)}
          className="w-9 h-9 rounded-xl flex items-center justify-center mb-4"
          style={{ background: 'hsl(var(--bg-elevated) / 0.9)' }}
        >
          <ChevronLeft size={20} className="text-text-primary" />
        </button>

        {challenge.is_featured && (
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full mb-2 bg-tint-yellow">
            <span className="text-accent-yellow text-[11px] font-bold">Featured Challenge</span>
          </div>
        )}

        <h1 className="text-text-primary text-2xl font-bold mb-1">{challenge.name}</h1>
      ...
      <div className="mx-4 -mt-4 rounded-2xl p-4 mb-4 bg-bg-card border border-border shadow-sm" style={{ boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
        <div className="text-center mb-3">
          <p className="text-text-muted text-xs mb-1">Total Prize Pool</p>
          <p className="text-text-primary font-bold" style={{ fontSize: '36px', lineHeight: 1 }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-3" style={{ borderTop: '1px solid hsl(var(--border-light))' }}>
          {[
            ...
            <div key={stat.label} className="text-center">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center mx-auto mb-1" style={{ background: 'hsl(var(--bg-input))', color: theme.accent }}>
                {stat.icon}
              </div>
      ...
      <div className="mx-4 mb-4 rounded-2xl p-4 bg-bg-card border border-border shadow-sm">
        <div className="flex justify-between items-center mb-2">
          <p className="text-text-primary text-sm font-bold">Spots Filling</p>
        ...
        <div className="w-full h-3 rounded-full" style={{ background: 'hsl(var(--bg-input))' }}>
          <div
        ...
        {challenge.is_almost_full && (
          <p className="text-error text-xs font-semibold mt-2 text-center">Almost full, join before it's too late!</p>
        )}
      </div>

      <div className="mx-4 mb-4 rounded-2xl p-4 bg-bg-card border border-border shadow-sm">
        <p className="text-text-primary text-sm font-bold mb-3">How Payouts Work</p>
        {[
      ...
      <div
        ...
        style={{ background: 'hsl(var(--bg-page) / 0.95)', backdropFilter: 'blur(8px)', borderTop: '1px solid hsl(var(--border-light))' }}
      >
        {challenge.user_is_joined ? (
          <div className="flex flex-col gap-2">
            <div className="w-full py-3.5 rounded-xl text-center text-sm font-bold bg-tint-green text-accent-green">
          ...
          <div className="flex flex-col gap-2">
            <div className="w-full py-3.5 rounded-xl text-center text-sm font-bold bg-bg-card text-text-muted border border-border">
