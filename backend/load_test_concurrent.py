...
class Stats:
    ...
    def record_status(self, status_code: int) -> None:
        with self.lock:
            self.http_status_counts[status_code] = (
                self.http_status_counts.get(status_code, 0) + 1
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--base-url", default=os.getenv("LOAD_BASE_URL", "http://127.0.0.1:8000")
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.1,
        help="Delay between sync loops (seconds)",
    )
    parser.add_argument(
        "--password", default=os.getenv("LOAD_TEST_PASSWORD", "Step2Win!234")
    )
    parser.add_argument(
        "--user-prefix", default=os.getenv("LOAD_USER_PREFIX", "load_user")
    )
...
def build_signature(
    user_id: str, timestamp: str, body_bytes: bytes, secret: str
) -> str:
...
def run_worker(
    ...
    try:
        ...
        if register_res.status_code in (200, 201):
            ...
            errors.append(
                f"register:{register_res.status_code}:{register_res.text[:160]}"
            )
        ...
        for loop in range(args.loops):
            ...
            if args.signing_secret and user_id:
                sync_headers["X-App-Signature"] = build_signature(
                    user_id, ts, body, args.signing_secret
                )
            ...
            if sync_res.status_code == 200:
                ...
                sync_fail += 1
                errors.append(
                    f"sync[{loop}]:{sync_res.status_code}:{sync_res.text[:160]}"
                )
        ...
        if args.chat_challenge_id:
            ...
            if not chat_post_ok:
                errors.append(
                    f"chat_post:{chat_post.status_code}:{chat_post.text[:160]}"
                )
...
def main() -> None:
    ...
    print(
        f"Users: {args.users}, loops/user: {args.loops}, chat challenge: {args.chat_challenge_id}"
    )
    ...
    with ThreadPoolExecutor(max_workers=args.users) as executor:
        futures = [
            executor.submit(run_worker, i, args, run_id, stats)
