...
def test_api_endpoints():
    ...
    try:
        resp = request_with_retry(
            "GET", f"{base_url}/api/auth/profile/", headers=headers
        )
        print(f"✅ User Profile: {resp.status_code}")
        profile = resp.json()
        print(
            f"   User: {profile.get('username')}, XP: {profile.get('xp_profile', {}).get('total_xp', 'N/A')}"
        )
    except Exception as e:
    ...
    try:
        resp = request_with_retry(
            "GET", f"{base_url}/api/gamification/xp/my_xp/", headers=headers
        )
        print(f"✅ XP Profile: {resp.status_code}")
        if resp.status_code == 200:
            xp_data = resp.json()
            print(
                f"   Level: {xp_data.get('level', 'N/A')}, Total XP: {xp_data.get('total_xp', 0)}"
            )
    except Exception as e:
    ...
    try:
        resp = request_with_retry(
            "GET", f"{base_url}/api/gamification/events/", headers=headers
        )
    ...
    try:
        resp = request_with_retry(
            "GET", f"{base_url}/api/admin/users/", headers=headers
        )
