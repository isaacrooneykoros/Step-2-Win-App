...
def generate_hmac_signature(
    user_id: int, body: str, secret: str
) -> tuple[str, str, str]:
    """
...
def test_valid_signed_sync():
    ...
    sig, timestamp, idempotency_key = generate_hmac_signature(
        user.id, sync_body, app_secret
    )
...
def test_invalid_signature():
    ...
    sig, timestamp, idempotency_key = generate_hmac_signature(
        user.id, sync_body, wrong_secret
    )
...
def test_idempotency():
    ...
    if response2.status_code == 409:
        ...
        print(
            f"❌ Expected 409 or {response1.status_code}, got {response2.status_code}"
        )
