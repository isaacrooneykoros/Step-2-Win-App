...
_CURRENT_MANAGEMENT_COMMAND = (
    sys.argv[1] if len(sys.argv) > 1 and sys.argv[0].endswith("manage.py") else ""
)
...
if not DEBUG and not _ALLOW_BOOTSTRAP_SECRET_FALLBACKS:
    if SECRET_KEY.startswith("django-insecure-") or len(SECRET_KEY) < 50:
        raise ImproperlyConfigured(
            "Production SECRET_KEY must be random and at least 50 characters long."
        )
    if any(host in {"localhost", "127.0.0.1", "testserver"} for host in ALLOWED_HOSTS):
        raise ImproperlyConfigured(
            "Production ALLOWED_HOSTS cannot contain localhost/testserver values."
        )

if (
    ENVIRONMENT == "production"
    and os.getenv("USE_SQLITE", "False").strip().lower() == "true"
):
    raise ImproperlyConfigured(
        "SQLite is not allowed in production. Configure PostgreSQL DATABASE_URL/DATABASE_POOL_URL."
    )
if ENVIRONMENT == "production" and not os.getenv("REDIS_URL", "").strip():
    raise ImproperlyConfigured(
        "REDIS_URL is required in production for throttling/locks/channels/celery."
    )
USE_REDIS = (
    os.getenv("USE_REDIS", "True" if os.getenv("REDIS_URL") else "False") == "True"
)
ENABLE_DEFENDER = (
    os.getenv("ENABLE_DEFENDER", "True" if os.getenv("REDIS_URL") else "False")
    == "True"
)
...
MEDIA_BASE_URL = (
    (os.getenv("MEDIA_BASE_URL") or os.getenv("RENDER_EXTERNAL_URL") or "")
    .strip()
    .rstrip("/")
)
...
CORS_ALLOWED_ORIGINS = list(
    dict.fromkeys(
        _CORS_REQUIRED_NATIVE
        + _CORS_REQUIRED_WEB
        + (_cors_explicit if _cors_explicit else _CORS_DEFAULT_DEV)
    )
)
...
DEFAULT_CELERY_BROKER_URL = (
    os.getenv("REDIS_URL", "sqla+sqlite:///" + str(BASE_DIR / "celery_broker.sqlite3"))
    if USE_REDIS
    else "sqla+sqlite:///" + str(BASE_DIR / "celery_broker.sqlite3")
)
...
if not DEBUG:
    SECURE_SSL_REDIRECT = (
        os.getenv("SECURE_SSL_REDIRECT", "True").strip().lower() == "true"
    )
    ...
    SECURE_HSTS_PRELOAD = (
        os.getenv("SECURE_HSTS_PRELOAD", "True").strip().lower() == "true"
    )
...
if not ADMIN_URL:
    ADMIN_URL = (
        f"admin-{secrets.token_urlsafe(12).replace('-', '').replace('_', '').lower()}/"
    )
...
INTASEND_WEBHOOK_SECRET = (
    os.getenv("INTASEND_WEBHOOK_SECRET", "").strip() or SECRET_KEY
)  # Webhook challenge secret
...
MIN_TRUST_SCORE_FOR_PAID_CHALLENGE = int(
    os.getenv("MIN_TRUST_SCORE_FOR_PAID_CHALLENGE", "60")
)
MIN_CHALLENGES_JOINED_TO_CREATE_PAID_CHALLENGE = int(
    os.getenv("MIN_CHALLENGES_JOINED_TO_CREATE_PAID_CHALLENGE", "1")
)

# Anti-cheat v2 rollout flags and thresholds
STEP_ANTICHEAT_V2_ENABLED = (
    os.getenv("STEP_ANTICHEAT_V2_ENABLED", "False").strip().lower() == "true"
)
STEP_ANTICHEAT_V2_SHADOW_MODE = (
...
ANTICHEAT_V2_SUSPICIOUS_CADENCE_SPM = float(
    os.getenv("ANTICHEAT_V2_SUSPICIOUS_CADENCE_SPM", "205")
)
ANTICHEAT_V2_IMPOSSIBLE_CADENCE_SPM = float(
    os.getenv("ANTICHEAT_V2_IMPOSSIBLE_CADENCE_SPM", "245")
)
ANTICHEAT_V2_SUSPICIOUS_BURST_5S = int(
    os.getenv("ANTICHEAT_V2_SUSPICIOUS_BURST_5S", "18")
)
ANTICHEAT_V2_IMPOSSIBLE_BURST_5S = int(
    os.getenv("ANTICHEAT_V2_IMPOSSIBLE_BURST_5S", "28")
)
ANTICHEAT_V2_REVIEW_RISK = float(os.getenv("ANTICHEAT_V2_REVIEW_RISK", "45"))
ANTICHEAT_V2_PAYOUT_HOLD_RISK = float(os.getenv("ANTICHEAT_V2_PAYOUT_HOLD_RISK", "70"))
ANTICHEAT_V2_REJECT_RISK = float(os.getenv("ANTICHEAT_V2_REJECT_RISK", "90"))

# Wallet balance management
MAX_LOCKED_BALANCE_PERCENT = int(
    os.getenv("MAX_LOCKED_BALANCE_PERCENT", "80")
)  # Max 80% of wallet can be locked
...
RECON_MAX_NEGATIVE_BALANCE_USERS = int(
    os.getenv("RECON_MAX_NEGATIVE_BALANCE_USERS", "0")
)
RECON_MAX_CALLBACK_FAILURE_RATE_PCT = float(
    os.getenv("RECON_MAX_CALLBACK_FAILURE_RATE_PCT", "5.0")
)
...
ANTICHEAT_DRIFT_PER_SAMPLE_ALERT_PCT = float(
    os.getenv("ANTICHEAT_DRIFT_PER_SAMPLE_ALERT_PCT", "35.0")
)
ANTICHEAT_DRIFT_MAX_AVG_ABS_DELTA_PCT = float(
    os.getenv("ANTICHEAT_DRIFT_MAX_AVG_ABS_DELTA_PCT", "20.0")
)
ANTICHEAT_DRIFT_MAX_HIGH_DRIFT_RATIO_PCT = float(
    os.getenv("ANTICHEAT_DRIFT_MAX_HIGH_DRIFT_RATIO_PCT", "25.0")
)
ANTICHEAT_DRIFT_MAX_REVIEW_MISMATCH_RATIO_PCT = float(
    os.getenv("ANTICHEAT_DRIFT_MAX_REVIEW_MISMATCH_RATIO_PCT", "10.0")
)
