...
def main():
    ...
    try:
        ...
        if any(f["flag_type"] == "impossible_rate" for f in r.flags):
            print("PASS: impossible_rate check works")
        else:
        ...
        if (
            any(f["flag_type"] == "daily_cap" for f in r.flags)
            and r.approved_steps == 60000
        ):
            print("PASS: daily_cap and step capping works")
        else:
            print(
                f"FAIL: daily_cap={any(f['flag_type'] == 'daily_cap' for f in r.flags)}, approved_steps={r.approved_steps}"
            )
        ...
        if any(f["flag_type"] == "distance_too_low" for f in r.flags):
            print("PASS: distance_too_low check works")
        else:
