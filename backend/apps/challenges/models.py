...
class Challenge(models.Model):
    ...
    max_participants = models.IntegerField(
        default=50, validators=[MinValueValidator(2), MaxValueValidator(1000)]
    )
    ...
    is_private = models.BooleanField(
        default=False, help_text="Private challenges require invite code"
    )
    ...
    featured_until = models.DateTimeField(
        null=True, blank=True, help_text="Auto-unfeature after this datetime."
    )
...
class Participant(models.Model):
    ...
    steps = models.IntegerField(
        default=0, help_text="Total steps accumulated during challenge"
    )
    qualified = models.BooleanField(
        default=False, help_text="Whether user met the milestone requirement"
    )
    ...
    rank = models.IntegerField(
        null=True, blank=True, help_text="Final rank in the challenge"
    )
...
class ChallengeResult(models.Model):
    ...
    challenge = models.ForeignKey(
        Challenge, on_delete=models.PROTECT, related_name="results"
    )
    participant = models.ForeignKey(
        "Participant", on_delete=models.PROTECT, related_name="result"
    )
    ...
    tied_with_count = models.IntegerField(
        default=0, help_text="Number of other participants this user tied with."
    )
    ...
    def __str__(self):
        return (
            f"{self.challenge.name} | {self.user.username} | "
...
class ChallengeMessage(models.Model):
    ...
    challenge = models.ForeignKey(
        Challenge, on_delete=models.CASCADE, related_name="messages"
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
    ...
    is_system = models.BooleanField(
        default=False, help_text="True for automated event messages"
    )
