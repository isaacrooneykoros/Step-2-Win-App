...
@extend_schema(
    ...
    responses={
        201: ChallengeDetailSerializer,
        400: inline_serializer(
            name="CreateChallengeBadRequest",
            fields={
...
def create_challenge(request):
    ...
    with transaction.atomic():
        ...
        Participant.objects.create(challenge=challenge, user=user)
        ...
        result_serializer = ChallengeDetailSerializer(
    ...
    try:
        with transaction.atomic():
            ...
            if challenge.participants.count() >= challenge.max_participants:
                return Response(
                    {"error": "Challenge is full"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if user.locked_balance + challenge.entry_fee > max_lockable:
                return Response(
                    {
                        "error": (
                            f"Cannot join - would exceed max locked balance. "
                            f"Currently locked: KES {user.locked_balance}, "
                            f"Max allowed: KES {max_lockable}"
                        )
                    },
            ...
            # Add participant
            participant = Participant.objects.create(challenge=challenge, user=user)
...
def leaderboard(request, pk):
    ...
    participants = challenge.participants.select_related("user").order_by(
        "-steps", "joined_at"
    )
...
@extend_schema(
    ...
    responses={
        201: ChallengeDetailSerializer,
        400: inline_serializer(
            name="RematchBadRequest", fields={"error": serializers.CharField()}
        ),
    },
)
@api_view(["POST"])
...
@extend_schema(
    request=inline_serializer(
        name="ChallengeChatSendRequest", fields={"content": serializers.CharField()}
    ),
    responses={
        200: inline_serializer(
            name="ChallengeChatResponse",
            fields={
        ...
        201: inline_serializer(
            name="ChallengeChatSentResponse",
            fields={
...
def challenge_chat(request, pk):
    ...
    if request.method == "GET":
        ...
        message = ChallengeMessage.objects.create(
            challenge=challenge, user=request.user, message=content, is_system=False
        )
...
@extend_schema(
    responses={
        200: inline_serializer(
            name="ChallengeSocialStatsResponse",
            fields={
                "most_consistent": serializers.DictField(allow_null=True),
                "biggest_single_day": serializers.DictField(allow_null=True),
                "most_improved": serializers.DictField(allow_null=True),
            },
        )
    },
)
...
def challenge_social_stats(request, pk):
    ...
    if duration >= 4:
        ...
        for p in participants:
            early_steps = (
                HealthRecord.objects.filter(
                    user=p.user, date__gte=challenge.start_date, date__lt=midpoint
                ).aggregate(total=Sum("steps"))["total"]
                or 0
            )
            ...
            late_steps = (
                HealthRecord.objects.filter(
                    user=p.user, date__gte=midpoint, date__lte=challenge.end_date
                ).aggregate(total=Sum("steps"))["total"]
                or 0
            )
    ...
    return Response(
        {
            "most_consistent": (
                {
                    "username": (
                        most_consistent.user.username if most_consistent else None
                    ),
            ...
            "biggest_single_day": (
                {
                    "username": (
                        biggest_single_day.user.username if biggest_single_day else None
                    ),
                    "steps": biggest_day_steps,
                    "date": str(biggest_day_date) if biggest_day_date else None,
                }
                if biggest_single_day
                else None
            ),
            "most_improved": (
                {
...
def public_lobby(request):
    ...
    if sort == "pool":
        ...
        qs = qs.annotate(participant_count=Count("participants")).order_by(
            "-participant_count", "-total_pool"
        )
    else:
    ...
    challenges = list(qs)
    serializer = LobbyCardSerializer(
...
@extend_schema(
    ...
    responses={
        ...
        404: inline_serializer(
            name="LobbyCardNotFound", fields={"error": serializers.CharField()}
        ),
    },
)
@api_view(["GET"])
...
@extend_schema(
    responses={
        200: inline_serializer(
            name="SpectatorLeaderboardResponse",
            fields={
                "challenge": serializers.DictField(),
                "leaderboard": serializers.ListField(),
                "qualified_count": serializers.IntegerField(),
                "total_participants": serializers.IntegerField(),
                "user_is_participant": serializers.BooleanField(),
            },
        )
    }
)
...
def spectator_leaderboard(request, pk):
    ...
    participants = list(
        challenge.participants.select_related("user").order_by("-steps")
    )
    ...
    qualified = [
        ...
        if participant.steps >= challenge.milestone
    ]
    ...
    def _serialize_result(r):
        ...
        return {
            ...
            "milestone_reached_at": (
                r.milestone_reached_at.isoformat() if r.milestone_reached_at else None
            ),
        }
...
@extend_schema(
    responses={
        200: inline_serializer(
            name="MyRecentResultsResponse",
            fields={
                "has_results": serializers.BooleanField(),
                "challenge": serializers.DictField(required=False),
                "my_result": serializers.DictField(required=False),
                "leaderboard": serializers.ListField(required=False),
