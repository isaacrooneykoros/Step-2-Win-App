...
def create_challenge(request):
    ...
    with transaction.atomic():
        ...
        # Deduct entry fee, lock it, and increment challenges_joined in a single database call
...
def join_challenge(request):
    ...
    try:
        with transaction.atomic():
            ...
            # Fetch and lock user first to prevent race condition/double-spend on wallet checks
            user = request.user.__class__.objects.select_for_update().get(id=request.user.id)
...
def rematch_challenge(request, pk):
    ...
    with transaction.atomic():
        ...
        # Validate available_balance and MAX_LOCKED_BALANCE_PERCENT limit within transaction
        if user.available_balance < entry_fee:
            return Response(
        ...
        if user.locked_balance + entry_fee > max_lockable:
            return Response(
                {
                    'error': (
                        f'Cannot start rematch - would exceed max locked balance. '
                        f'Currently locked: KES {user.locked_balance}, '
                        f'Max allowed: KES {max_lockable}'
                    )
                },
