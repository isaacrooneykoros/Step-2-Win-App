...
def create_challenge(request):
    ...
    with transaction.atomic():
        # Lock user object for balance update to prevent race conditions
        ...
        if entry_fee > 0:
            ...
            if user.challenges_joined < min_joined:
                return Response(
...
def join_challenge(request):
    ...
    try:
        with transaction.atomic():
            # Lock user and challenge objects to prevent race conditions
            user = request.user.__class__.objects.select_for_update().get(id=request.user.id)
        ...
        data = [
            {
                ...
                'content': sanitize_chat_message(m.message) if not m.is_system else m.message,
