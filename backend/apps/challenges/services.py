...
def cancel_challenge(challenge, reason=""):
    ...
    from apps.challenges.models import ChallengeResult, Participant
    ...
    with transaction.atomic():
        ...
        logger.info(
            f'Cancelling challenge {challenge.id}: "{challenge.name}". Reason: {reason or "admin action"}'
        )

        # Release locked balance for all participants
        participants = Participant.objects.filter(challenge=challenge).select_related(
            "user"
        )
        for participant in participants:
            ...
            logger.info(
                f"Refunded KES {challenge.entry_fee} to {user.username} for cancelled challenge"
            )
    ...
    with transaction.atomic():
        ...
        for r in resolved:
            ...
            if r.payout_method == "refund":
                ...
                user.locked_balance -= challenge.entry_fee
                user.save(
                    update_fields=["wallet_balance", "locked_balance", "updated_at"]
                )
                ...
                WalletTransaction.objects.create(
                    ...
                    balance_after=user.wallet_balance,
                    description=(
                        f"Refund: {challenge.name} — " f"no participants qualified"
                    ),
...
def _payout_description(challenge, resolved_participant) -> str:
    ...
    if r.payout_method == "proportional":
        ...
        if r.tied_with_count > 0:
            return (
                f"{base} — tied with {r.tied_with_count} other(s), "
...
def finalize_expired_challenges(today=None):
    ...
    expired = Challenge.objects.filter(
        status="active", end_date__lt=target_date
