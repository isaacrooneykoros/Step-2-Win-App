...
def _refresh_token() -> str:
    ...
    raise last_error or PochiPayAPIError(
        "PochPay token refresh failed for unknown reason"
    )
...
def initiate_mpesa_collection(
    ...
    try:
        ...
        if data.get("errors") or data.get("message") == "error":
            raise ValueError(
                f"PochPay collection error: {data.get('message', data.get('errors'))}"
            )

        logger.info(
            f"M-Pesa STK Push sent | order_id={order_id} | phone={phone_number} | amount={amount}"
        )
    ...
    try:
        ...
        if data.get("errors") or data.get("message"):
            ...
            if not data.get("result", {}).get("isProcessing"):
                raise ValueError(
                    f"PochPay disbursement error: {data.get('message', data.get('errors'))}"
                )

        logger.info(
            f"Disbursement batch sent | request_id={request_id} | recipients={len(recipients)}"
        )
        return data
...
def send_withdrawal_to_mobile(
    ...
    try:
        ...
        logger.info(
            f"Withdrawal sent to M-Pesa | ref={tracking_reference} | amount={amount}"
        )
        return data
    except requests.RequestException as e:
...
def send_withdrawal_to_bank(
    ...
    try:
        ...
        logger.info(
            f"Withdrawal sent to bank | ref={tracking_reference} | bank={bank_code}"
        )
        return data
    except requests.RequestException as e:
