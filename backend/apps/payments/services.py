...
def verify_intasend_signature(request, callback_name: str) -> None:
    ...
    if not secret:
        raise ImproperlyConfigured(
            "INTASEND_WEBHOOK_SECRET is not configured; webhook callbacks are disabled."
        )

    provided_sig = (request.headers.get("X-IntaSend-Signature", "") or "").strip()
    if not provided_sig:
        raise PaymentsServiceError(
            f"{callback_name}: missing webhook signature", status_code=403
        )
    ...
    if not hmac.compare_digest(provided_sig, expected_sig):
        raise PaymentsServiceError(
            f"{callback_name}: invalid webhook signature", status_code=403
        )
...
def request_withdrawal(user, data: dict[str, Any]) -> WithdrawalRequest:
    ...
    if method not in ("mpesa", "bank", "paybill"):
        raise PaymentsServiceError(
            "method must be one of: mpesa, bank, paybill", status_code=400
        )
    ...
    if method == "mpesa":
        ...
        try:
            ...
            raise PaymentsServiceError(
                "Could not validate bank. Try again.", status_code=502
            ) from exc
    else:
...
def reject_withdrawal_request(
    withdrawal: WithdrawalRequest, *, reason: str, reviewer
) -> WithdrawalRequest:
    with db_transaction.atomic():
        locked_withdrawal = WithdrawalRequest.objects.select_for_update().get(
            id=withdrawal.id
        )
        if locked_withdrawal.status != "pending_review":
            raise PaymentsServiceError(
                f"Cannot reject — status is: {locked_withdrawal.status}",
                status_code=400,
            )
...
def approve_withdrawal_and_send(
    ...
    with db_transaction.atomic():
        ...
        if locked_withdrawal.status != "pending_review":
            raise PaymentsServiceError(
                f"Cannot approve — status is: {locked_withdrawal.status}",
                status_code=400,
            )
        ...
        payout_txn = PaymentTransaction.objects.create(
            ...
            or locked_withdrawal.account_number
            or locked_withdrawal.short_code,
            narration=remarks,
        )
    ...
    try:
        if locked_withdrawal.method == "mpesa":
            ...
            raise PaymentsServiceError(
                f"Unknown withdrawal method: {locked_withdrawal.method}",
                status_code=400,
            )
    except Exception as exc:
        with db_transaction.atomic():
            locked_user = (
                locked_withdrawal.user.__class__.objects.select_for_update().get(
                    id=locked_withdrawal.user_id
                )
            )
            locked_user.wallet_balance = (
    ...
    with db_transaction.atomic():
        ...
        locked_withdrawal.save(
            update_fields=["tracking_reference", "request_id", "status", "updated_at"]
        )

        payout_txn.tracking_reference = tracking_id
        payout_txn.request_id = tracking_id
        payout_txn.save(
            update_fields=["tracking_reference", "request_id", "updated_at"]
        )
...
def log_callback(
    callback_type: str, payload: dict[str, Any], order_id: str = ""
) -> CallbackLog:
...
def process_deposit_callback(payload: dict[str, Any]) -> None:
    ...
    with db_transaction.atomic():
        txn = PaymentTransaction.objects.select_for_update().get(
            order_id=order_id, type="deposit"
        )
        ...
        if state == "COMPLETE":
            locked_user = txn.user.__class__.objects.select_for_update().get(
                id=txn.user_id
            )
            balance_before = locked_user.wallet_balance
