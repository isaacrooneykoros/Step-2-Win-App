...
def run_financial_reconciliation(
    *, thresholds: ReconciliationThresholds | None = None, send_alerts: bool = True
) -> dict[str, Any]:
    ...
    unprocessed_callbacks = CallbackLog.objects.filter(
        processed=False, created_at__lt=now - timedelta(minutes=5)
    ).count()
    callbacks_24h = CallbackLog.objects.filter(created_at__gte=since_24h)
    callbacks_total_24h = callbacks_24h.count()
    callback_failures_24h = callbacks_24h.filter(
        processed=False, created_at__lt=now - timedelta(minutes=5)
    ).count()
    callback_failure_rate_pct = (
        (callback_failures_24h / callbacks_total_24h) * 100.0
        if callbacks_total_24h
        else 0.0
    )
    ...
    duplicate_request_rejections_today = {
        scope: get_duplicate_rejection_count(scope) for scope in SCOPES
    }
    ...
    if duplicate_gateway_refs:
        breaches.append(
            f"duplicate_gateway_references_detected={len(duplicate_gateway_refs)}"
        )
