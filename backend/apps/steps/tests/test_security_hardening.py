...
from apps.steps.anti_cheat import finalize_step_session, score_session
from apps.steps.models import (AntiCheatPolicy, DeviceRegistration,
                               StepSession, StepSyncEvent,
                               SuspiciousSessionReview, UserTrustProfile)
from apps.steps.security import (compute_payload_hash, create_session_token,
                                 detect_replay, get_active_policy,
                                 get_active_policy_version,
                                 get_or_create_user_trust_profile,
                                 get_trust_reward_modifier, hash_session_token,
                                 is_session_expired, is_session_valid,
...
class SecurityUtilsTestCase(TestCase):
    ...
    def test_compute_payload_hash(self):
        ...
        payload3["ml_walk_probability"] = 0.850001
...
class ReplayDetectionTestCase(TransactionTestCase):
    ...
    def test_detect_duplicate_client_event_id(self):
        ...
        is_replay, reason = detect_replay(
            self.user.id, str(self.session.id), event_id, 2, "hash2"
        )
    ...
    def test_detect_duplicate_sequence_number(self):
        ...
        is_replay, reason = detect_replay(
            self.user.id, str(self.session.id), "event-2", 1, "hash2"
        )
...
class UserTrustProfileTestCase(TestCase):
    ...
    def test_get_or_create_trust_profile(self):
        ...
        self.assertEqual(profile.trust_tier, "standard")
    ...
    def test_trust_score_update_clean_session(self):
        ...
        update_user_trust_after_session(
            self.user, session_risk_score=10.0, is_replay=False
        )
    ...
    def test_trust_score_update_replay(self):
        ...
        update_user_trust_after_session(
            self.user, session_risk_score=80.0, is_replay=True
        )
