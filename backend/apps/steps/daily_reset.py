...
def get_today_health_record(user):
    ...
    record, created = HealthRecord.objects.get_or_create(
        user=user, date=today, defaults={"steps": 0}
    )
...
def update_streak(user):
    ...
    records = HealthRecord.objects.filter(user=user, date__lte=today).values(
        "date", "steps"
    )
    ...
    checking_date = (
        today if today_steps >= MIN_STEPS_FOR_STREAK else today - timedelta(days=1)
    )
...
def get_streak_status(user):
    ...
    last_active = (
        HealthRecord.objects.filter(
            user=user, date__lt=today, steps__gte=MIN_STEPS_FOR_STREAK
        )
        .order_by("-date")
        .first()
    )
