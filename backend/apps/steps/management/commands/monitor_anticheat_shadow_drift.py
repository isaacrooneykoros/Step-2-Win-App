...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--no-alerts",
            action="store_true",
            help="Do not send external webhook alerts",
        )
        parser.add_argument(
            "--lookback-hours",
            type=int,
            default=int(getattr(settings, "ANTICHEAT_DRIFT_LOOKBACK_HOURS", 24)),
        )
        parser.add_argument(
            "--min-samples",
            type=int,
            default=int(getattr(settings, "ANTICHEAT_DRIFT_MIN_SAMPLES", 50)),
        )
        parser.add_argument(
            "--per-sample-alert-pct",
            type=float,
            default=float(
