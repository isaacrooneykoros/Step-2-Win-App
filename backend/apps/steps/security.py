...
def detect_replay(
    ...
    if session_id and sequence_number:
        ...
        if existing:
            return (
                True,
                f"Duplicate sequence_number {sequence_number} in session {session_id}",
            )
...
def is_session_valid(session) -> bool:
    ...
    return session.status == "active" and not is_session_expired(session)
...
def update_user_trust_after_session(
    user, session_risk_score: float, is_replay: bool = False
) -> None:
    """
    ...
    profile.save(
        update_fields=[
            "trust_score",
            "verified_sessions_count",
            "suspicious_sessions_count",
            "replay_attempts_count",
            "last_suspicious_at",
            "last_verified_at",
            "updated_at",
        ]
