...
from apps.core.throttles import (DashboardReadRateThrottle,
                                 DepositRateThrottle, WithdrawalRateThrottle)
...
from apps.payments.services import initiate_deposit as initiate_deposit_service
...
def wallet_summary(request):
    ...
    total_deposited = transactions.filter(type="deposit").aggregate(Sum("amount"))[
        "amount__sum"
    ] or Decimal("0.00")

    total_withdrawn = abs(
        transactions.filter(type="withdrawal").aggregate(Sum("amount"))["amount__sum"]
        or Decimal("0.00")
    )

    total_earned = transactions.filter(type="payout").aggregate(Sum("amount"))[
        "amount__sum"
    ] or Decimal("0.00")
...
def deposit(request):
    ...
    try:
        trans = initiate_deposit_service(
            request.user, amount, request.user.phone_number
        )
...
def transaction_stats(request):
    ...
    for choice in WalletTransaction.TYPE_CHOICES:
        ...
        total = transactions.filter(type=type_code).aggregate(Sum("amount"))[
            "amount__sum"
        ] or Decimal("0.00")
        stats_by_type[type_code] = {"count": count, "total": str(total)}
    ...
    return Response(
        {
            ...
            "latest_transaction": (
                TransactionSerializer(transactions.first()).data
