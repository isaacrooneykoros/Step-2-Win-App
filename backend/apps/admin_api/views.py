...
class AdminUserViewSet(viewsets.ModelViewSet):
    ...
    def ban_user(self, request, pk=None):
        ...
        AuditLog.log_action(
            ...
            description=f'Admin banned user {user.username}',
            request=request
        )
    ...
    def unban_user(self, request, pk=None):
        ...
        AuditLog.log_action(
            ...
            resource_id=user.id,
            resource_name=user.username,
            description=f'Admin unbanned user {user.username}',
            request=request
        )
...
class AdminChallengeViewSet(viewsets.ModelViewSet):
    ...
    def reject_challenge(self, request, pk=None):
        ...
        challenge = self.get_object()
        reason = str(request.data.get('reason', 'No reason provided')).strip()
        from django.core.exceptions import ValidationError as DjangoValidationError
        try:
    ...
    def update_challenge(self, request, pk=None):
        ...
        if name:
            from django.core.exceptions import ValidationError as DjangoValidationError
            try:
                challenge.name = sanitize_text(name, max_length=100)
            except DjangoValidationError as e:
                return Response({'error': str(e.message)}, status=status.HTTP_400_BAD_REQUEST)
...
def update_support_ticket(request, ticket_id):
    ...
    if 'admin_notes' in request.data:
        notes = str(request.data.get('admin_notes') or '').strip()
        from django.core.exceptions import ValidationError as DjangoValidationError
        try:
            updates['admin_notes'] = sanitize_text(notes)
        except DjangoValidationError as e:
            return Response({'error': str(e.message)}, status=status.HTTP_400_BAD_REQUEST)
