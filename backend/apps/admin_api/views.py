...
class AdminChallengeViewSet(viewsets.ModelViewSet):
    ...
    def reject_challenge(self, request, pk=None):
        ...
        try:
            # Sanitize rejection reason to prevent Stored XSS
            reason = sanitize_text(reason)
        except DjangoValidationError as e:
            return Response({'error': f'Invalid reason: {e.message}'}, status=status.HTTP_400_BAD_REQUEST)
...
def reply_support_ticket(request, ticket_id):
    ...
    try:
        # Sanitize admin reply to prevent Stored XSS
...
def update_support_ticket(request, ticket_id):
    ...
    if 'admin_notes' in request.data:
        ...
        try:
            # Sanitize admin notes to prevent Stored XSS
            updates['admin_notes'] = sanitize_text(admin_notes)
        except DjangoValidationError as e:
            return Response({'error': f'Invalid admin_notes: {e.message}'}, status=status.HTTP_400_BAD_REQUEST)
...
def action_flag(request, flag_id):
    ...
    if admin_note:
        try:
            # Sanitize admin note to prevent Stored XSS
            admin_note = sanitize_text(admin_note)
        except DjangoValidationError as e:
            return Response({'error': f'Invalid admin_note: {e.message}'}, status=status.HTTP_400_BAD_REQUEST)
