...
class AdminChallengeViewSet(viewsets.ModelViewSet):
    ...
    def reject_challenge(self, request, pk=None):
        ...
        challenge = self.get_object()
        reason = sanitize_text(request.data.get('reason', 'No reason provided'), max_length=500)
...
def reply_support_ticket(request, ticket_id):
    ...
    message_text = sanitize_text(request.data.get('message', ''), max_length=5000)
    if not message_text:
...
def update_support_ticket(request, ticket_id):
    ...
    if 'admin_notes' in request.data:
        updates['admin_notes'] = sanitize_text(request.data.get('admin_notes') or '', max_length=1000)
...
def action_flag(request, flag_id):
    ...
    admin_note = sanitize_text(request.data.get('admin_note') or '', max_length=500)
...
def reject_withdrawal(request, withdrawal_id):
    ...
    """
    reason = sanitize_text(request.data.get('reason', 'Rejected by admin'), max_length=500)
