...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    def _latest_moderation_flag(self, obj):
        ...
        latest = (
            FraudFlag.objects.filter(user=obj, reviewed=True)
            .order_by("-created_at")
            .first()
        )
    ...
    def get_moderation_message(self, obj) -> str:
        ...
        return defaults.get(
            action, "Your flagged step activity has been reviewed by the admin team."
        )
...
class LoginSerializer(serializers.Serializer):
    ...
    username = serializers.CharField(
        required=True, help_text="Username, email, or phone number"
    )
...
class SupportTicketCreateSerializer(serializers.Serializer):
    ...
    category = serializers.ChoiceField(
        choices=SupportTicket.CATEGORY_CHOICES, default="general"
    )
    priority = serializers.ChoiceField(
        choices=SupportTicket.PRIORITY_CHOICES, default="medium"
    )
